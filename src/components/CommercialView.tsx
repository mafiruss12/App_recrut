...
export function CommercialView({
  ...
  const [personalClients, setPersonalClients] = useState<ClientEntry[]>([]);
  const [liveDuplicateCheck, setLiveDuplicateCheck] = useState<DuplicateCheckResult>({ isDuplicate: false });

  // Load personal history from Supabase
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const list = await storage.getClientsByCommercial(currentUser.id);
        if (!cancelled) setPersonalClients(list);
      } catch {
        if (!cancelled) setPersonalClients([]);
      }
    })();
    return () => { cancelled = true; };
  }, [currentUser.id, pendingCount]);

  // Debounced live duplicate check
  useEffect(() => {
    const clean = normalizePhone(phoneInput);
    if (clean.length < 8) {
      setLiveDuplicateCheck({ isDuplicate: false });
      return;
    }
    const t = setTimeout(async () => {
      const res = await storage.checkDuplicate(clean);
      setLiveDuplicateCheck(res);
    }, 400);
    ...
    <div className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full z-10 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="backdrop-blur-md bg-white/5 border border-white/10 p-4 sm:p-5 rounded-2xl">
          ...
          <div id="historique-saisies" className="text-2xl sm:text-3xl font-black text-white">{personalClients.length}</div>
